...
*/
		...
		{
				currTime=(System.DateTime.Now-starttime).ToString();		//faceMat1.material.SetColor("_Color",Color.blue);
		if(_PecCard.pecStarted)
			{
				leftPlayerEngagementsecondHalf++;
					//turnTable[topRow][column1]=currTime;
			}
		else
			{
				leftPlayerEngagement++;
...
if(!(p1TotalEngagement>=20f)/&&!(p1TotalEngagement>=20f&&p2TotalEngagement<=15f))
			...
			*/
			...
			if(/*p2TotalEngagement<=10f*/angleBtwn2<=40f &&rightPlayerPosState==PositionState.Begin) /*&& !(p1TotalEngagement>=20f&&p2TotalEngagement<=15f)*/
			{
				...
				if(p2State!=null)
				p2State.text=rightPlayerPosState.ToString();
				turnTableTwo[topRowTwo,col1]=rightPlayerPosState.ToString()+" " +angleBtwn2.ToString();//p2TotalEngagement.ToString();
				turnTableTwo[topRowTwo,col2]=currTime;
				topRowTwo++;
			...
			else if(/*p2TotalEngagement<=10f*/angleBtwn2<=40f&&rightPlayerPosState==PositionState.FacingForward) /*&& !(p1TotalEngagement>=20f&&p2TotalEngagement<=15f)*/
			{
				...
				currTime=(System.DateTime.Now-starttime).ToString();
				//faceMat2.material.SetCo
				//_Color("_Color",Color.blue);
				if(_PecCard.pecStarted)//second phase of level
				{
					rightPlayerEngagementsecondHalf++;
				}
